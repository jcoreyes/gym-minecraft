...
BLOCK_KEY = dict(air=0, brick_block=1, grass=2, dirt=3, clay=4)
...
class MinecraftEnvRLKit(gym.Env):
    ...
    def __init__(self, mission_file):
        ...
        full_dim = self.x * self.y * self.z


        self.partial_obs_space = Box(low=0, high=10, shape=(self.z,), dtype=np.float32)
        self.obs_space = Box(low=0, high=10, shape=(full_dim+4,), dtype=np.float32)
    ...
    def reset(self):
        ...
        while world_state.number_of_observations_since_last_state < 3:
            time.sleep(0.1)
    ...
    def _get_world_state(self):
        ...
        self.agent_host.getWorldState()
        world_state = self.agent_host.getWorldState()
        while world_state.number_of_observations_since_last_state < 5 and world_state.is_mission_running:
            time.sleep(0.01)
            world_state = self.agent_host.peekWorldState()
        return world_state
    ...
    def _get_obs(self, world_state):
        ...
        full_obs = np.concatenate([state_obs, agent_pos], -1)
        #import pdb; pdb.set_trace()
        partial_obs = state_obs.reshape((self.y, self.x, self.z))[0, :, -1]
        return dict(
    ...
    def step(self, action):
        ...
        reward = self.compute_rewards(action, obs)
    ...
    def compute_rewards(self, actions, obs):
        ...
        r = np.sum(achieved_goals == desired_goals, -1)  - 50
...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        while not done and (steps < 1000):
            ...
            #key_to_action = dict(w=0, s=1, d=2, a=3, e=4)
            ...
            # print("done: " + str(done))
            #print("obs: " + str(obs))
            print(obs['state_observation'][:-4].reshape((env.y, env.x, env.z)))
            print(obs['state_observation'][-4:])
